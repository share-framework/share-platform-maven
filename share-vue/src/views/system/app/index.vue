      ...
      <el-row>
        <div style="height: 300px; overflow-x: auto;">
          <el-form ref="menuForm" :show="false" :rules="rules" :model="menu" label-width="80px">
            ...
            <el-form-item label="菜单名称" prop="appName">
              <el-input v-model="menu.appName" :readonly="seeShow"></el-input>
            </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleAdd(menu) {
      ...
      this.menuParentName = this.multipleSelection[0].appName
      this.method = 'add'
      ...
      if (that.method === 'add') {
        ...
        addMenu(this.menu).then(response => {
          ...
          if (code === 200) {
            this.$notify({
              title: '成功通知',
              message: `[ ${that.menu.appName} ] 菜单` + data,
              type: 'success'
            })
          ...
          this.$notify({
            title: '失败通知',
            message: `[ ${that.menu.appName} ] 菜单` + error,
            type: 'error'
          })
        })
      } else {
    ...
    handleSelectionChange(val) {
      ...
      if (val.length !== 0) {
        ...
        this.menu.appName = val[val.length - 1].appName
